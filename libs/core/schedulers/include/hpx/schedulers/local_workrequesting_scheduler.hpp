...
namespace hpx { namespace threads { namespace policies {
    ...
    class HPX_CORE_EXPORT local_workrequesting_scheduler : public scheduler_base
    {
    public:
    ...
    private:
        ...
        struct task_data
        {
            ...
            std::vector<thread_id_ref_type> tasks_;
        };
    ...
    public:
        ...
        bool is_core_idle(std::size_t num_thread) const override
        {
            if (num_thread < num_queues_ &&
                data_[num_thread].data_.queue_->get_queue_length() != 0)
            {
                return false;
            }
            if (num_thread < num_high_priority_queues_ &&
            ...
            if (d.tasks_->get(&thrds))
            {
                ...
                if (!thrds.tasks_.empty())
                {
                    ...
                    if (next_thrd != nullptr)
                    {
                        ...
                        // immediately
                        ++d.num_recent_tasks_executed_;
                        *next_thrd = HPX_MOVE(thrds.tasks_.back());
                    }
                    else
                    {
                        d.queue_->schedule_thread(
                            HPX_MOVE(thrds.tasks_.back()), true);
                        d.queue_->increment_num_stolen_to_pending();
                        ++added;
                    }
